...
- 或 有效的云端 API Key（DeepSeek/Kimi等）
...
然后在游戏设置中选择对应模型名，URL会自动为你填写。

#### ☁️ 选项三：使用云端 API（追求最佳效果）

- 服务商：选择丰富，基本涵盖所有海内外服务商
- Base URL：游戏设置会根据你的选择**自动填写**
- 模型：每一家服务商的所有模型均考虑在内，用户可以**自行选择**
...
| `display` | `text_speed` | 文本显示速度（0.0~0.1）            |
| `audio`   | `bgm_volume` | BGM 音量（-20~20）                |
